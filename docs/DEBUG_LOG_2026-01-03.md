...
|--------|---------------------|-------------------|
| Accuracy after 10K seqs | ~20% | 99% |
| Best consecutive | 6 | 337 |
| Gate response to marker=1 | 0.29 | 0.95 |
| Gate response to marker=0 | 0.28 | 0.05 |

### Training Speed
- ~112 sequences/second on M1 Mac
- 100K sequences takes ~15 minutes
- Did not reach 2000 consecutive in available time, but trajectory suggests success
...
## Conclusion
The 1997 LSTM Adding Problem requires either:
1. **Gate pre-training**: Initialize gates to respond to the marker signal
2. **Very long training**: 100K+ sequences with slow gradient accumulation
3. **Lucky initialization**: Specific weight combinations that allow gradient flow

The failure mode (predicting mean) occurs because input gates remain closed, preventing the network from selectively storing marked values. With gate pre-training, the model achieves 99% accuracy after just 10K sequences.
