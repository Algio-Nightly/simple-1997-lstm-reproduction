...
### Update (January 10th, 2026)
We have now confirmed that **gate pre-training is NOT strictly necessary** for convergence. A paper-exact run (SGD lr=0.5, 93 weights, no pre-training) successfully learned the task around sequence 45,000 and achieved a test max error of 0.0387 after 100,000 sequences. This proves the architecture is robust enough to learn the marker response from random initialization, provided enough sequences are used.
